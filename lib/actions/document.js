...
/**
 * Lấy tài liệu kèm danh sách quy tắc
 * @param {number} id - ID của tài liệu
 * @returns {Promise<Object>} Response từ API
 */
export async function getDocumentWithRules(id) {
  try {
    if (!id) {
      return {
        status: 'error',
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/admin/documents/${id}/with-rules`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || 'Lấy thông tin tài liệu và quy tắc thất bại',
        data: null
      };
    }
