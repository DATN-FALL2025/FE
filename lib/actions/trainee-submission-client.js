 ...
 * @param {string} submissionData.requireDocumentName - Tên tài liệu yêu cầu
...
export async function createTraineeSubmission(submissionData) {
  try {
    ...
    if (!documentID || !traineeApplicationId || !requireDocumentName || !submissionDocumentFile) {
      console.error('❌ Missing required fields');
      return {
        status: 'error',
        message: 'Thiếu thông tin bắt buộc: documentID, traineeApplicationId, requireDocumentName, hoặc submissionDocumentFile',
        data: null
      };
    }
    ...
    const url = `${API_BASE_URL}/trainee_submission/create_trainee_submission_by_trainee`;
