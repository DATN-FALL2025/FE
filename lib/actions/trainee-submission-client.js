 ...
 * @param {File|File[]} submissionData.submissionDocumentFile - File hoặc mảng files tài liệu cần nộp
...
export async function createTraineeSubmission(submissionData) {
  try {
    ...
    console.log('📋 Extracted data:', {
      ...
      isArray: Array.isArray(submissionDocumentFile),
      fileCount: Array.isArray(submissionDocumentFile) ? submissionDocumentFile.length : 1,
    });
    ...
    formData.append('requiredDocumentName', submissionName);
    if (takeNote) {
      formData.append('takeNote', takeNote);
    }
    
    // Handle single file or multiple files
    if (Array.isArray(submissionDocumentFile)) {
      // Multiple files
      submissionDocumentFile.forEach((file) => {
        formData.append('submissionDocumentFile', file);
      });
      console.log('📦 FormData created with', submissionDocumentFile.length, 'files');
    } else {
      // Single file
      formData.append('submissionDocumentFile', submissionDocumentFile);
      console.log('📦 FormData created with 1 file');
    }
    ...
    if (error.name === 'AbortError') {
      return {
        status: 'error',
        message: 'Request timeout - Server phản hồi quá chậm. Vui lòng thử lại.',
        data: null
      };
    }
 ...
 * @param {string} updateData.requiredDocumentName - Tên tài liệu yêu cầu (required)
 * @param {string} [updateData.newTakeNote] - Ghi chú mới
 * @param {File|File[]} [updateData.newSubmissionDocumentFile] - File hoặc mảng files tài liệu mới
...
export async function updateTraineeSubmission(submissionId, updateData) {
  try {
    ...
    console.log('📋 Extracted data:', {
      ...
      isArray: Array.isArray(newSubmissionDocumentFile),
      fileCount: Array.isArray(newSubmissionDocumentFile) ? newSubmissionDocumentFile.length : (newSubmissionDocumentFile ? 1 : 0),
    });
    ...
    if (newSubmissionDocumentFile) {
      if (Array.isArray(newSubmissionDocumentFile)) {
        // Multiple files
        newSubmissionDocumentFile.forEach((file) => {
          formData.append('newSubmissionDocumentFile', file);
        });
        console.log('📦 FormData created with', newSubmissionDocumentFile.length, 'files');
      } else {
        // Single file
