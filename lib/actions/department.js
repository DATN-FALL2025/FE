...
/**
 * Lấy danh sách tất cả khoa (Version 2 - cho Training Director)
 * @returns {Promise<Object>} Response từ API
 */
export async function getAllDepartmentsV2() {
  try {
    const response = await fetch(
      `${API_BASE_URL}${API_PATH}/admin/departments/departments-v2`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error('Error getting all departments v2:', error);
    return {
      status: 'error',
      message: error.message || 'Lỗi kết nối đến server',
      data: null
