...
export async function authenticateAccount(loginData) {
  try {
    ...
    const url = `${API_BASE_URL}/api/account/v1/authenticateAccount?${queryParams}`;
...
export async function createUser(accountData) {
  try {
    ...
    const url = `${API_BASE_URL}/api/account/v1/createUser?${queryParams}`;
...
export async function getAllUsers() {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/account/v1/getAllUser`,
      {
        method: 'GET',
        headers: {
...
export async function createRole(roleData) {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/account/v1/createRole?${queryParams}`,
      {
        method: 'POST',
        headers: {
          'accept': '*/*',
...
export async function getAllRoles() {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/account/v1/getAllRole`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
/**
 * Import nhiều tài khoản từ file Excel
 * @param {Array} accounts - Mảng các tài khoản
 * @param {string} accounts[].userName - Tên đăng nhập
 * @param {string} accounts[].password - Mật khẩu
 * @param {string} accounts[].gmail - Email
 * @param {string} [accounts[].positionName] - Tên vị trí
 * @param {string} [accounts[].departmentName] - Tên phòng ban
 * @returns {Promise<Object>} Response từ API
 */
export async function importUsers(accounts) {
  try {
    ...
    const url = `${API_BASE_URL}/api/account/multipleAccounts`;
    console.log('📤 Import Multiple Accounts Request:', url, requestBody);
    ...
    console.log('📤 Import Multiple Accounts Response:', {
      status: response.status,
      ok: response.ok,
      data: data
    });
    ...
    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || 'Import tài khoản thất bại',
        data: null
      };
    }
    ...
    return {
      status: 'success',
      message: data.message || `Import thành công ${accounts.length} tài khoản`,
      data: data.data
    };
  } catch (error) {
