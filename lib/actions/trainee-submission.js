...
/**
 * Lấy thông tin đợt nộp đang hoạt động (active batch)
 *
 * @returns {Promise<Object>} Response từ API
 */
export async function getActiveBatch() {
  try {
    console.log('📅 Fetching active batch info');

    const response = await fetch(
      `${API_BASE_URL}/api/batch/active-batch`,
      {
        method: 'GET',
        headers: {
          'accept': '*/*',
    ...
    const data = await response.json();
    console.log('📅 Get active batch response:', data);

    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || 'Lấy thông tin đợt nộp đang hoạt động thất bại',
        data: null
      };
    }
...
/**
 * Approve or Reject individual trainee submission (từng tài liệu)
 * PUT /api/trainee_submission/approve-reject-status/{submissionId}
 *
 * @param {number} submissionId - ID của submission
 * @param {string} status - Status: "Pending" | "Approve" | "Reject" | "Complete" | "InProgress"
 * @param {string} [reason] - Lý do (bắt buộc nếu Reject)
 * @param {string|null} [token] - Auth token from client
 * @returns {Promise<Object>} Response từ API
 */
export async function approveRejectSubmission(submissionId, status, reason = '', token = null) {
  try {
    console.log('🔄 Approve/Reject Submission:', { submissionId, status, reason });
    ...
    // Validate status
    const validStatuses = ['Pending', 'Approve', 'Reject', 'Complete', 'InProgress'];
    ...
    const url = `${API_BASE_URL}/api/trainee_submission/approve-reject-status/${submissionId}?${params.toString()}`;
    ...
    const data = await response.json();
    console.log('🔄 Approve/Reject Submission Response:', data);

    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || `${status === 'Approve' ? 'Duyệt' : 'Từ chối'} tài liệu thất bại`,
        data: null
      };
    }
...
/**
 * Approve or Reject entire trainee application (cho ACADEMIC_STAFF_AFFAIR)
 * PUT /api/trainee_application/{traineeApplicationId}/comeplete-reject
 *
 * @param {number} traineeApplicationId - ID của trainee application
 * @param {string} statusEnum - Status: "Pending" | "Approve" | "Reject" | "Complete" | "InProgress"
 * @param {string} [rejectReason] - Lý do từ chối (bắt buộc nếu Reject)
 * @param {string|null} [token] - Auth token from client
 * @returns {Promise<Object>} Response từ API
 */
export async function approveRejectTraineeApplication(traineeApplicationId, statusEnum, rejectReason = '', token = null) {
  try {
    console.log('🔄 Approve/Reject Trainee Application:', { traineeApplicationId, statusEnum, rejectReason });
    ...
    // Require reason if rejecting
    if (statusEnum === 'Reject' && !rejectReason) {
      return {
        status: 'error',
        message: 'Vui lòng nhập lý do từ chối',
        data: null
    ...
    // Build query params
    const params = new URLSearchParams();
    params.append('statusEnum', statusEnum);

    const url = `${API_BASE_URL}/api/trainee_application/${traineeApplicationId}/comeplete-reject?${params.toString()}`;
    console.log('🔄 Request URL:', url);
    ...
    const data = await response.json();
    console.log('🔄 Approve/Reject Application Response:', data);

    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || `${statusEnum === 'Approve' ? 'Duyệt' : 'Từ chối'} đơn đăng ký thất bại`,
        data: null
      };
    }
    ...
    console.error('Error approving/rejecting trainee application:', error);
    return {
      status: 'error',
      message: error.message || 'Lỗi kết nối đến server',
      data: null
