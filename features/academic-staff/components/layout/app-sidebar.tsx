...
export function AppSidebar() {
  ...
  const userEmail = user?.gmail || user?.email || "staff@example.com";
  ...
  return (
    <Sidebar collapsible="icon">
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              ...
              asChild
            >
              <Link href="/academic-staff/approvals">
                <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground">
                  <BookOpen className="size-4" />
                </div>
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold">IDMAWA</span>
                  <span className="truncate text-xs">Nhân viên Học vụ</span>
                </div>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel>Quản lý học vụ</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {navItems.map((item) => {
                const isActive =
